...
async function swagataMobileScrapeSongLinks(req, res) {
  const givenArtist = req.body.artist;
  const browser = await puppeteer.launch({ headless: true });
  const page = await browser.newPage();
  const website = "https://swagatamobile.in"
  ...
  const songData = await page.evaluate((givenArtist,website) => {
    ...
    const items = container.querySelectorAll("div b");
    const data = [];
    ...
    items.forEach((el) => {
      ...
      const artist = fontTag?.innerText.trim();
...
export default swagataMobileScrapeSongLinks;
